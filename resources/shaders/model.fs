...
// Hardcoded directional light properties
	...
	// Ambient
    vec3 ambient = vec3(dir_light.ambient); // Adjust this for more/less ambient light
  ...
  // Normalize the normal
    ...
    // vec3 finalColor = tex_color * (ambient + diffuse);

	float shadow = ShadowCalculation(FragPosLightSpace);

	return (ambient + (1.0 - shadow) * (diffuse)) * tex_color;
	// return finalColor;
}
