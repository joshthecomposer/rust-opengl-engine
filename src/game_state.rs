...
use crate::{camera::Camera, shaders, some_data::{CUBE_POSITIONS, UNIT_CUBE_VERTICES}};
...
gl::FLOAT,
    ...
    pub fn render(&mut self) {
        ...
        unsafe {
            gl::ClearColor(0.1, 0.2, 0.33, 1.0);
            gl::Clear(gl::COLOR_BUFFER_BIT | gl::DEPTH_BUFFER_BIT);

            let main_shader_prog = *self.shaders.get("main").unwrap();

            gl::UseProgram(main_shader_prog);

            gl::ActiveTexture(gl::TEXTURE0);
            gl::BindTexture(gl::TEXTURE_2D, self.texture);

            let projection_c_string = CString::new("projection").unwrap().as_ptr();
            let view_c_string = CString::new("view").unwrap().as_ptr();
            let model_c_string = CString::new("model").unwrap().as_ptr();

            gl::UniformMatrix4fv(
                gl::GetUniformLocation(main_shader_prog, projection_c_string),
                1,
                gl::FALSE,
            ...
            gl::UniformMatrix4fv(
                gl::GetUniformLocation(main_shader_prog, view_c_string),
                1,
                gl::FALSE,
